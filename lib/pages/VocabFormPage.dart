...
class _VocabFormPage extends State<VocabFormPage>
  ...
  Future<bool> fillVocabForm(VocabBundle vocab,
      {bool doUpdateImage = false}) async {
    try {
      if (vocab == null) {
        return false;
      }
      ...
      for (int i = 0; i < vocab.definitionsBundle.length; i++) {
        ...
        formControllers[i].wordSampleSentenceController.text =
  ...
  Future<bool> autoFillForms() async {
    try {
      VocabBundle vocab =
          await FetchDataWordsAPI.requestFromAPI(wordController.value.text);
    ...
    return Form(
      ...
      child: ExpansionTile(
        ...
        children: <Widget>[
          ...
          buildInputField(
              "Sample Sentence", formController.wordSampleSentenceController),
