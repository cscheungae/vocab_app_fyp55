...
class _StudyPage extends State<StudyPage> with TickerProviderStateMixin {
  ...
  Future<VocabBundle> getvocabBundle(int index) async => await DatabaseProvider.instance.readVocabBundle(this._vocabList[index].vid );
  ...
  Future<void> submitRanking(int vid, double rating ) async {
    ...
    } catch (exception){ Toast.show("Oops! seems like something is wrong when submitting your rating", context); print(exception); }
  }
                  ...
                  child: ClipRRect( 
                    child: BackdropFilter(
                      ...
                      child: Container(
                        ...
                        child: Text(
                          (showBlur) ? "want a hint? Click to show the image" : "",
  ...
  Widget buildStartBottomBar(){ 
   ...
   Widget buildStudyVocabBottomBar(VocabBundle vb){ 
    return buildButton("I forgot...", "I know this word!", 
      () async{
        this.rating = 0;
  ...
  Widget build(BuildContext context){
    ...
    return Scaffold(
      ...
      body: (this._studyIndex < 0) ? this.buildStartPage() : this.buildFadeAnimatedPage(this.buildStudyVocabPage(this.vb)) ,
      bottomNavigationBar: 
        AnimatedContainer (
          height: 70,
          duration: Duration(milliseconds: 400),
          child: (this._studyIndex < 0) ? this.buildStartBottomBar() : ((this.showRating) ? this.buildStudyStarRatingBar(this.vb) : this.buildStudyVocabBottomBar(this.vb)  ) ,
      ),
    );
  }
